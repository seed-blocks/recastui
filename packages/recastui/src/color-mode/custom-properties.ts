...
import { ColorMode, ColorModesScale, Theme } from '../theme';

const toVarName = (key: string) => `--recast-ui-${key.replace('-__default', '')}`
...
export const __createColorStyles = (theme: Theme) => {
	...
	if (printColorModeName) {
		...
		if (printMode) {
			...
			console.error(
				`RecastUI \`printColorModeName\` was not found in colors scale`,
 ...
 * .recast-ui-<colormode> classes used for no-flash serverside rendering.
 */
export function __createColorProperties(
	...
	Object.keys(modes).forEach((mode) => {
		const className = `.recast-ui-${mode}`
